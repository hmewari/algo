...
from blueprints.protection import protection_bp  # Position protection (SL/Target/Trail/Brokerage)
...
def ensure_protection_tables_exists():
    """Initialize protection tables without requiring a static module import."""
    from importlib import import_module

    import_module("database.protection_db").init_db()
...
def create_app():
    ...
    app.register_blueprint(protection_bp)  # Register Position Protection blueprint
    ...
    with app.app_context():
        ...
        # Exempt position protection endpoints from CSRF (JSON API calls from React frontend)
        csrf.exempt(app.view_functions["protection.load_protections"])
        csrf.exempt(app.view_functions["protection.save_protection"])
        csrf.exempt(app.view_functions["protection.delete_protection_route"])
        csrf.exempt(app.view_functions["protection.get_protection_settings"])
        csrf.exempt(app.view_functions["protection.save_protection_settings"])
        csrf.exempt(app.view_functions["protection.get_analyze_mode"])
        ...
        # Keep it in the extension registry: the later startup workers retrieve
        # it from there. A local reference is passed to the order-update boot
        # worker below, so all boot tasks synchronize on this one Event.
        db_ready = _th.Event()
        app.extensions["db_ready"] = db_ready
        ...
        start_order_update_adapters_on_boot(db_ready=db_ready)
    ...
    def wait_for_db_ready():
        ...
        db_ready = app.extensions.get("db_ready")
        if db_ready is not None and not db_ready.is_set():
            db_ready.wait(timeout=30)
...
def setup_environment(app):
    ...
    if "db_ready" not in app.extensions:
        # Fallback for callers that invoke setup_environment() without first
        # going through create_app().
        app.extensions["db_ready"] = threading.Event()
    ...
    def _init_databases_and_schedulers():
        with app.app_context():
            ...
            db_init_functions = [
                ...
                ("Protection DB", ensure_protection_tables_exists),
