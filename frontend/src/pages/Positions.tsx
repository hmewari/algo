...
export default function Positions() {
  ...
  const { data: enhancedPositions, isLive, isPaused } = useLivePrice(positions, {
    ...
    multiQuotesRefreshInterval: 2000,      // REST fallback polls every 2s for faster protection updates
  ...
  useEffect(() => {
    ...
    enhancedPositions.forEach((position: any) => {
      ...
      if (shouldClose) {
        ...
        // Fire close order
        handleClosePosition(position).then((closed) => {
          if (closed) {
  ...
  const [qoUnderlying, setQoUnderlying] = useState<QoUnderlying>('NIFTY')
  const qoInstrument = QO_INSTRUMENTS.find((instrument) => instrument.value === qoUnderlying) ?? QO_INSTRUMENTS[0]
  ...
  useEffect(() => {
    if (qoOptions.length > 0 && !qoOptions.some((instrument) => instrument.value === qoUnderlying)) {
      setQoUnderlying(qoOptions[0].value)
    }
  }, [qoOptions, qoUnderlying])
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await oiProfileApi.getExpiries(qoInstrument.optionExchange, qoUnderlying)
        if (cancelled) return
  ...
  }, [qoUnderlying, qoInstrument.optionExchange])
  ...
  useEffect(() => {
    ...
    fetchChain()
    return () => { cancelled = true }
  }, [qoUnderlying, qoExpiry, apiKey, convertExpiryForQO, qoInstrument.underlyingExchange])
  ...
  const underlyingExchangeForLtp = qoInstrument.underlyingExchange
  ...
  const handleClosePosition = useCallback(async (position: Position | PositionWithPnlPercent): Promise<boolean> => {
    if (dailyLockout) { showToast.error('Daily loss limit reached — trading locked', 'positions'); return false }
    try {
  ...
  const positionsWithPnlPercent = useMemo<PositionWithPnlPercent[]>(() =>
    filteredPositions.map((pos: Position) => {
      ...
      // Keep Analyzer Mode's realized P&L when recalculating the open leg's
      // unrealized P&L from the latest price.
      const realizedPnl = Number(pos.today_realized_pnl) || 0
      const lotSize = Number(pos.lot_size) || 1
      const livePnl = realizedPnl + (liveLtp - avgPrice) * qty * lotSize
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row flex-wrap items-center gap-2 sm:gap-4 px-4 py-2 bg-background border rounded-md">
        ...
        <div className="flex flex-wrap items-center justify-start gap-2 sm:gap-4 w-full sm:w-auto">
          <Select value={qoUnderlying} onValueChange={v => setQoUnderlying(v as QoUnderlying)}>
            <SelectTrigger className="h-7 w-28 text-xs"><SelectValue /></SelectTrigger>
            <SelectContent>{qoOptions.map((instrument) => <SelectItem key={instrument.value} value={instrument.value}>{instrument.value}</SelectItem>)}</SelectContent>
          </Select>
